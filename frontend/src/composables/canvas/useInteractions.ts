import type { Link, Peer, Subnet, Tool } from '@/types/network'
...
export interface DragState {
  active: boolean, type: '' | 'peer' | 'subnet', id: string, offsetX: number, offsetY: number, containedPeers: string[], containedSubnets: string[]
}

export interface ResizeDrag {
  active: boolean, id: string, edge: string, left: number, top: number, right: number, bottom: number, minW: number, minH: number
}
...
export function createInteractions (getPeers: () => Peer[], getSubnets: () => Subnet[], getLinks: () => Link[], getZoom: () => number) {
  ...
  function hitTestPeer (pt: { x: number, y: number }) {
    ...
    for (let i = peers.length - 1; i >= 0; i--) {
      const n: any = peers[i]
      if (Math.hypot(pt.x - n.x, pt.y - n.y) <= 24) {
        return n
      }
    }
    return null
  ...
  function hitTestSubnet (pt: { x: number, y: number }) {
    ...
    hits.sort((a, b) => {
      const ap = Number.parseInt(a.cidr.split('/')[1] || '0', 10)
      const bp = Number.parseInt(b.cidr.split('/')[1] || '0', 10)
      if (ap !== bp) {
        return bp - ap
      } // larger prefix first
  ...
  function hitTestLink (pt: { x: number, y: number }) {
    ...
    function distPointToSeg (px: number, py: number, ax: number, ay: number, bx: number, by: number) {
      const vx = bx - ax, vy = by - ay; const wx = px - ax, wy = py - ay; const c1 = vx * wx + vy * wy; if (c1 <= 0) {
        return Math.hypot(px - ax, py - ay)
      }
      const c2 = vx * vx + vy * vy; if (c2 <= c1) {
        return Math.hypot(px - bx, py - by)
      } const t = c1 / c2; const ix = ax + vx * t, iy = ay + vy * t; return Math.hypot(px - ix, py - iy)
    }
    for (let i = links.length - 1; i >= 0; i--) {
      ...
      if ((l as any).kind === 'membership' || (l as any).kind === 'subnet-service') {
        ...
        if ((l as any).kind === 'membership' && peer.subnetId && peer.subnetId === subnet.id) {
          continue
        }
        ...
        if (vx === 0 && vy === 0) {
          ...
          const sx = Math.abs(vx) / (hw || 1e-6), sy = Math.abs(vy) / (hh || 1e-6); const t = Math.max(sx, sy) || 1; Bx = cx + vx / t; By = cy + vy / t
        }
      ...
      if ((l as any).kind === 'subnet-subnet') {
        ...
        if (vx === 0 && vy === 0) {
          Ax = cxA + hwA; Ay = cyA; Bx = cxB - hwB; By = cyB
        } else {
      ...
      if ((l as any).kind === 'admin-p2p') {
        const a = peers.find(p => p.id === l.fromId); const b = peers.find(p => p.id === l.toId); if (!a || !b) {
          continue
        }
        const d = distPointToSeg(pt.x, pt.y, a.x, a.y, b.x, b.y); if (d <= threshold) {
          return l
        } continue
      }
      if ((l as any).kind === 'admin-peer-subnet') {
        ...
        const cx = subnet.x, cy = subnet.y, hw = subnet.width / 2, hh = subnet.height / 2; const vx = cx - peer.x, vy = cy - peer.y; let Bx = cx, By = cy; if (vx !== 0 || vy !== 0) {
          const sx = Math.abs(vx) / (hw || 1e-6), sy = Math.abs(vy) / (hh || 1e-6); const t = Math.max(sx, sy) || 1; Bx = cx - vx / t; By = cy - vy / t
        }
        const d = distPointToSeg(pt.x, pt.y, peer.x, peer.y, Bx, By); if (d <= threshold) {
          return l
        } continue
      }
      if ((l as any).kind === 'admin-subnet-subnet') {
        const sA = subnets.find(s => s.id === l.fromId); const sB = subnets.find(s => s.id === l.toId); if (!sA || !sB) {
          continue
        }
        const cxA = sA.x, cyA = sA.y, hwA = sA.width / 2, hhA = sA.height / 2; const cxB = sB.x, cyB = sB.y, hwB = sB.width / 2, hhB = sB.height / 2; const vx = cxB - cxA, vy = cyB - cyA; let Ax = cxA, Ay = cyA, Bx = cxB, By = cyB; if (vx === 0 && vy === 0) {
          Ax = cxA + hwA; Ay = cyA; Bx = cxB - hwB; By = cyB
        } else {
          const tA = Math.max(Math.abs(vx) / (hwA || 1e-6), Math.abs(vy) / (hhA || 1e-6)) || 1; Ax = cxA + vx / tA; Ay = cyA + vy / tA; const tB = Math.max(Math.abs(vx) / (hwB || 1e-6), Math.abs(vy) / (hhB || 1e-6)) || 1; Bx = cxB - vx / tB; By = cyB - vy / tB
        }
        const d = distPointToSeg(pt.x, pt.y, Ax, Ay, Bx, By); if (d <= threshold) {
          return l
        } continue
      }
      const a = peers.find(p => p.id === l.fromId); const b = peers.find(p => p.id === l.toId); if (!a || !b) {
  ...
  function edgeAtPoint (sub: Subnet, pt: { x: number, y: number }) {
    ...
    if (pt.x >= left - threshold && pt.x <= left + threshold && pt.y >= top - threshold && pt.y <= bottom + threshold) {
      horizontal = 'w'
    }
    if (pt.x >= right - threshold && pt.x <= right + threshold && pt.y >= top - threshold && pt.y <= bottom + threshold) {
      horizontal = horizontal || 'e'
    }
    if (pt.y >= top - threshold && pt.y <= top + threshold && pt.x >= left - threshold && pt.x <= right + threshold) {
      vertical = 'n'
    }
    if (pt.y >= bottom - threshold && pt.y <= bottom + threshold && pt.x >= left - threshold && pt.x <= right + threshold) {
      vertical = vertical || 's'
    }
    const combo = (vertical + horizontal) as any
    if (['n', 's', 'e', 'w'].includes(combo)) {
      return combo
    }
  ...
  function subnetContains (a: string, b: string) {
  ...
  function mostSpecificSubnetForIp (ip: string) {
    ...
    return candidates.sort((a, b) => (Number.parseInt(b.cidr.split('/')[1] || '0') - Number.parseInt(a.cidr.split('/')[1] || '0')))[0]
  }

  function clampPeerToSubnet (peer: any) {
    if (peer.ip) {
      const match = mostSpecificSubnetForIp(peer.ip)
      if (match && peer.subnetId !== match.id) {
        peer.subnetId = match.id
      }
    }
    if (!peer.subnetId) {
      return
    ...
    const subnets = getSubnets()
    const s = subnets.find(s => s.id === peer.subnetId); if (!s) {
      return
    }
    const margin = 26
