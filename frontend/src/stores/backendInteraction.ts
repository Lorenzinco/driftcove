...
export interface RawTopologyNew {
  networks: Array<{ subnet: RawSubnet; peers: RawPeer[] }>;
  links: Array<Record<string, any>>;
}
export interface RawTopologyLegacy {
  networks: Array<Record<string, RawPeer[]>>;
  links: Array<Record<string, any>>;
}
...
export const useBackendInteractionStore = defineStore("backendInteraction", {
  ...
  actions: {
    ...
    buildTopologySignature(): string {
      ...
      const peers = [...net.peers]
        .map((p) => ({
          ...
          x: p.x,
          y: p.y,
          publicKey: (p as any).publicKey || "",
          presharedKey: (p as any).presharedKey || "",
          services: Object.keys(p.services || {})
            .sort()
            .map((k) => {
              const sv: any = (p.services as any)[k];
              return {
                name: k,
                port: sv?.port,
                dept: sv?.department,
      ...
      return JSON.stringify({ subnets, peers, links });
    },
    ...
    async fetchTopology(force = false) {
      ...
      try {
        ...
        const serviceLinksDict: Record<string, any[]> =
          topo.service_links || {};
        ...
        for (const addr of Object.keys(peersDict)) {
          ...
          for (const k of Object.keys(servicesRaw))
            services[k] = servicesRaw[k];
          const subnetId = contained[p.public_key] || null;
          const id = p.public_key; // raw public key as id
          const host = Object.values(services).some(
            (sv: any) => sv && typeof sv.port === "number" && !isNaN(sv.port),
          );
          const rx = typeof p.rx === "number" && !isNaN(p.rx) ? p.rx : 0;
          const tx = typeof p.tx === "number" && !isNaN(p.tx) ? p.tx : 0;
          const lastHandshake =
            typeof p.last_handshake === "number" && !isNaN(p.last_handshake)
              ? p.last_handshake
              : 0;
          peersMeta.push({
            id,
        ...
        for (const baseAddr of Object.keys(p2pLinksDict)) {
          ...
          for (const n of neighbors) {
            ...
            const aId = basePeer.publicKey!,
              bId = neighborPeer.publicKey!;
        ...
        for (const serviceName of Object.keys(serviceLinksDict)) {
          ...
          const hostPeer = peersMeta.find(
            (p) => p.services && p.services[serviceName],
          );
          if (!hostPeer) continue;
          for (const c of consumers) {
            if (
              c?.public_key &&
              c.public_key !== hostPeer.publicKey &&
              peerByPublic[c.public_key]
            ) {
              links.push({
                id: `link_${linkCounter++}`,
                fromId: hostPeer.id,
        ...
        for (const cidr of Object.keys(subnetToServiceDict)) {
          ...
          for (const svc of servicesArr) {
            const svcName =
              svc && (svc.name || (typeof svc === "string" ? svc : ""));
            if (!svcName) continue;
            const hostPeer = peersMeta.find(
              (p) => p.services && p.services[svcName],
            );
            if (!hostPeer) continue;
            links.push({
              id: `link_${linkCounter++}`,
        ...
        for (const srcAddr of Object.keys(adminPeerToPeer)) {
          ...
          for (const tgt of adminPeerToPeer[srcAddr] || []) {
            ...
            const tgtPeer = peersMeta.find(
              (p) => p.publicKey === tgt.public_key,
            );
            if (!tgtPeer) continue;
            links.push({
              id: `link_${linkCounter++}`,
        ...
        this.topology = {
          ...
          links: links.map((l) => ({
            ...
            toId: l.toId,
            kind: l.kind,
            serviceName: (l as any).serviceName,
          })),
        };
        // Pass through rx/tx/lastHandshake so the network store can update live stats
        useNetworkStore().applyBackendTopology({
          peers: peersMeta.map((p) => ({
            id: p.id,
            name: p.name,
            ip: p.ip,
            subnetId: p.subnetId,
        ...
        // Update baseline signature after successful fetch
    ...
    buildCurrentTopologyPayload() {
      ...
      for (const p of netStore.peers) {
        ...
        for (const [svcName, svc] of Object.entries(p.services || {})) {
          services[svcName] = {
            name: svcName,
            department: (svc as any).department,
            port: (svc as any).port,
      ...
      for (const l of netStore.links.filter((l) => l.kind === "membership")) {
        ...
        if (peer && subnet) {
          ...
          if (pobj && !arr.some((x) => x.public_key === pobj.public_key))
            arr.push(pobj);
        }
      }
      ...
      function addAdj(baseAddr: string, neighborObj: any) {
        ...
        if (!arr.some((p) => p.public_key === neighborObj.public_key))
          arr.push(neighborObj);
      }
      ...
      for (const l of netStore.links.filter((l) => l.kind === "service")) {
        const svcName = (l as any).serviceName;
        if (!svcName) continue;
        const consumer = netStore.peers.find((p) => p.id === l.toId);
        if (!consumer) continue;
        const consumerObj = peerObjById[consumer.id];
        if (!consumerObj) continue;
        const arr = service_links[svcName] || (service_links[svcName] = []);
        if (!arr.some((x) => x.public_key === consumerObj.public_key))
          arr.push(consumerObj);
      }

      // subnet links dict
      const subnet_links: Record<string, any[]> = {};
      for (const [cidr, arr] of Object.entries(subnetMembers))
        subnet_links[cidr] = arr;
      ...
      (function buildSubnetToSubnet() {
        ...
        const seen = new Set<string>();
        for (const l of netStore.links.filter(
          (l) => l.kind === "subnet-subnet",
        )) {
          const a = netStore.subnets.find((s) => s.id === l.fromId);
          const b = netStore.subnets.find((s) => s.id === l.toId);
          if (!a || !b) continue;
          const pk = pairKey(a.cidr, b.cidr);
          if (seen.has(pk)) continue;
          seen.add(pk);
          const arrA =
            subnet_to_subnet_links[a.cidr] ||
            (subnet_to_subnet_links[a.cidr] = []);
          arrA.push({
            subnet: b.cidr,
            name: b.name,
      ...
      for (const l of netStore.links.filter(
        (l) => l.kind === "subnet-service",
      )) {
        const subnet =
          netStore.subnets.find((s) => s.id === l.toId) ||
          netStore.subnets.find((s) => s.id === l.fromId);
        ...
        if (!arr.some((s: any) => (s && s.name) === svcDef.name))
          arr.push(svcDef);
      }
    ...
    async connectAdminPeerToSubnet(adminUsername: string, subnetCidr: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to connect admin peer to subnet";
        return false;
      }
    },
    ...
    async disconnectAdminPeerFromSubnet(
      ...
      try {
        ...
        this.lastError =
          e?.message || "Failed to disconnect admin peer from subnet";
        return false;
      }
    },
    async connectPeers(peer1: string, peer2: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to connect peers";
        return false;
      }
    },
    async disconnectPeers(peer1: string, peer2: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to disconnect peers";
        return false;
      }
    },
    async connectAdminPeerToPeer(adminUsername: string, peerUsername: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to create admin peer link";
        return false;
      }
    },
    async disconnectAdminPeerFromPeer(
      ...
      try {
        ...
        this.lastError = e?.message || "Failed to remove admin peer link";
        return false;
      }
    },
    async connectPeerToService(username: string, serviceName: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to connect peer to service";
        return false;
      }
    },
    async disconnectPeerFromService(username: string, serviceName: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to disconnect peer from service";
        return false;
      }
    },
    async connectSubnetToSubnet(subnetA: string, subnetB: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to connect subnets";
        return false;
      }
    },
    async connectAdminSubnetToSubnet(adminSubnet: string, subnet: string) {
      ...
      try {
        ...
        this.lastError = e?.message || "Failed to connect admin subnets";
        return false;
      }
    },
    async disconnectSubnetFromSubnet(subnetA: string, subnetB: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to disconnect subnets";
        return false;
      }
    },
    async disconnectAdminSubnetFromSubnet(adminSubnet: string, subnet: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to disconnect admin subnet link";
        return false;
      }
    },
    async connectSubnetToService(subnetCidr: string, serviceName: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to connect subnet to service";
        return false;
      }
    },
    async disconnectSubnetFromService(subnetCidr: string, serviceName: string) {
      try {
        ...
        this.lastError =
          e?.message || "Failed to disconnect subnet from service";
        return false;
      }
    },
    async fetchPeerConfig(username: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to fetch config";
        return null;
      }
    },
    ...
    async deleteSubnet(subnetCidr: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to delete subnet";
        return false;
      }
    },
    ...
    async deleteSubnetWithPeers(subnetCidr: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to delete subnet with peers";
        return false;
      }
    },
    ...
    async deletePeer(username: string) {
      try {
        ...
        this.lastError = e?.message || "Failed to delete peer";
        return false;
      }
    },
    ...
    async getNftTableRules() {
      try {
        ...
        this.lastError = e?.message || "Failed to fetch nftables rules";
        return [];
      }
    },
  },
  getters: {
    ...
    topologyDirty(state): boolean {
      ...
      if (state.lastFetchedTopologySig == null) return false;
