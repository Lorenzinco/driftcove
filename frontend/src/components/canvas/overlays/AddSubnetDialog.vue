      ...
      <v-card-text class="pt-4">
        <v-form @submit.prevent="submit">
          ...
          <v-text-field
            ...
            density="comfortable"
            :error="!!cidrError"
            :error-messages="cidrError ? [cidrError] : []"
            label="CIDR (e.g. 10.0.2.0/24)"
            required
            variant="outlined"
          />
          <v-textarea
          ...
          <v-alert v-if="error" class="mt-4" density="comfortable" type="error">{{ error }}</v-alert>
        </v-form>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" @click="close">Cancel</v-btn>
        <v-btn color="primary" :disabled="!canSubmit" :loading="loading" @click="submit">Create</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
<script setup lang="ts">
  ...
  const form = reactive({ name: '', subnet: '', description: '', width: 420, height: 260, rgba: 0x00_FF_00_25 })
  ...
  watch(rgbaHex, h => {
    if (/^0x[0-9a-fA-F]{8}$/.test(h)) {
      const val = Number.parseInt(h, 16) >>> 0
      form.rgba = val >>> 0
  ...
  function validateCidr (raw: string) {
    if (!raw) {
      cidrError.value = 'CIDR is required'; return
    }
    const m = raw.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)\/(\d{1,2})$/)
    if (!m) {
      cidrError.value = 'Format must be a.b.c.d/len'; return
    }
    const oct = m.slice(1, 5).map(n => Number.parseInt(n, 10))
    const bits = Number.parseInt(m[5], 10)
    if (oct.some(o => o < 0 || o > 255)) {
      cidrError.value = 'Octets must be 0-255'; return
    }
    if (bits < 0 || bits > 32) {
      cidrError.value = 'Mask must be 0-32'; return
    }
    ...
    if (parentCidr.value) {
      if (!cidrContains(parentCidr.value, raw)) {
        cidrError.value = `Must be inside parent ${parentCidr.value}`; return
      }
      const parentBits = Number.parseInt(parentCidr.value.split('/')[1], 10)
      if (bits <= parentBits) {
        cidrError.value = `Mask must be more specific than parent (/${parentBits})`; return
      }
    }
    cidrError.value = ''
  }
  ...
  function showAt (worldX: number, worldY: number) {
    ...
    candidates.sort((a, b) => Number.parseInt(b.cidr.split('/')[1]) - Number.parseInt(a.cidr.split('/')[1]))
  ...
  async function submit () {
    ...
    try {
      ...
      const parent = netStore.subnets
        ...
        .sort((a, b) => Number.parseInt(b.cidr.split('/')[1]) - Number.parseInt(a.cidr.split('/')[1]))[0]
      ...
      error.value = error_?.message || 'Failed to create subnet'
    } finally {
      loading.value = false
    }
  }
