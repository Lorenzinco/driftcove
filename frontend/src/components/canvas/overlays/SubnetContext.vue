<template>
  ...
  />
  <div v-if="menu.open" class="subnet-menu-backdrop" @click="hideMenu" @mousedown.stop />
  <SubnetInfoDialog
    ref="infoDialog"
    v-model="infoOpen"
    @create-peer="openAddPeer(menu.subnetId)"
    @show-peer="onShowPeer"
    @show-subnet="onShowSubnet"
  />
...
<script setup lang="ts">
  ...
  function openAddPeer (id: string) {
    hideMenu(); addPeerDialog.value?.show(id)
  }

  function openInfo (id: string) {
    hideMenu(); infoDialog.value?.show(id)
  }
  ...
  function onKey (e: KeyboardEvent) {
    if (e.key === 'Escape' && menu.open) hideMenu()
  }
  function onExternalDeleteRequest (e: any) {
    const id = e?.detail?.id; if (!id) return; confirmDelete(id)
  }
  onMounted(() => {
    window.addEventListener('keydown', onKey); window.addEventListener('subnet-context-request-delete', onExternalDeleteRequest as any)
  })
  onUnmounted(() => {
    window.removeEventListener('keydown', onKey); window.removeEventListener('subnet-context-request-delete', onExternalDeleteRequest as any)
  })
