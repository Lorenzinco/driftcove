  ...
  <v-dialog v-model="open" max-width="460">
    <v-card>
      ...
      <v-card-text class="pt-4">
        ...
        <v-alert v-if="error" class="mb-3" density="comfortable" type="error">{{
          error
        }}</v-alert>
        <!-- Peer ↔ Peer: choose between p2p or service link -->
        <v-radio-group
          v-if="fromType === 'peer' && toType === 'peer'"
          v-model="mode"
          class="mt-2"
        >
          <v-radio value="p2p">
            ...
            <template #label>
              <div class="d-flex align-center">
                ...
                <span class="ms-2">{{
                  toIsHost
                    ? "Peer to Service"
                    : "Peer to Service (destination has no services)"
                }}</span>
              </div>
            </template>
          </v-radio>
        ...
        <!-- Peer → Subnet: choose between public (peer-subnet) or admin privilege (admin-peer-subnet) -->
        <v-radio-group
          v-if="fromType === 'peer' && toType === 'subnet'"
          v-model="mode"
          class="mt-2"
        >
        ...
        <v-radio-group
          v-if="fromType === 'subnet' && toType === 'peer'"
          v-model="mode"
          class="mt-2"
        >
          <v-radio value="peer-subnet">
            ...
            <template #label>
              <div class="d-flex align-center">
                ...
                <span class="ms-2">{{
                  toIsHost
                    ? "Subnet to Service"
                    : "Subnet to Service (destination has no services)"
                }}</span>
              </div>
            </template>
          </v-radio>
        ...
        <v-expand-transition>
          <div
            v-if="mode === 'service' || mode === 'subnet-service'"
            class="mt-2"
          >
            <v-select
        ...
        <div v-if="mode === 'admin-p2p'" class="mt-2">
          <v-alert
            color="purple"
            density="comfortable"
            :text="
        ...
        <div v-if="mode === 'subnet-subnet'" class="mt-2">
          <v-alert
            density="comfortable"
            text="Subnet to Subnet linking means every peer from one subnet can communicate with every peer from another subnet."
            type="info"
          />
        </div>
        <div v-if="mode === 'admin-subnet-subnet'" class="mt-2">
          <v-alert
            density="comfortable"
            text="Admin Subnet → Subnet grants initiation from every member of the admin subnet to all members of the target subnet regardless of their public flags."
            type="info"
          />
        </div>
      </v-card-text>
