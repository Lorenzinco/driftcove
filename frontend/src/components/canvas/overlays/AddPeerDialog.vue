<template>
  <v-dialog v-model="open" max-width="520">
    <v-card>
      <v-card-title class="d-flex justify-space-between align-center">
        <span>Add Peer to {{ subnetLabel }}</span>
        <v-btn icon="mdi-close" variant="text" @click="close()" />
      </v-card-title>
      <v-divider />
      <v-card-text class="text-body-2">
        <v-alert
          v-if="error"
          class="mb-3"
          density="comfortable"
          :text="error"
          type="error"
        />
        <v-form ref="formRef" @submit.prevent="submit">
          <v-text-field
            v-model="username"
            autofocus
            density="comfortable"
            :disabled="loading"
            label="Username"
            required
          />
          <v-text-field
            v-model="address"
            density="comfortable"
            :disabled="loading"
            label="Specific IP (optional)"
            placeholder="e.g. 10.0.10.42"
          />
          <div class="text-caption text-medium-emphasis mb-2">Leave IP blank to auto-assign the next available address.</div>
          <v-btn
            block
            color="primary"
            :loading="loading"
            prepend-icon="mdi-account-plus"
            type="submit"
          >Add Peer</v-btn>
        </v-form>
      </v-card-text>
      <v-divider />
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" @click="close()">Cancel</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
<script setup lang="ts">
  import { computed, ref } from 'vue'
  import { useBackendInteractionStore } from '@/stores/backendInteraction'
  import { useNetworkStore } from '@/stores/network'

  const store = useNetworkStore()
  const backend = useBackendInteractionStore()

  const open = ref(false)
  const targetSubnetId = ref<string | null>(null)
  const username = ref('')
  const address = ref('')
  const loading = ref(false)
  const error = ref('')
  const formRef = ref()

  const subnet = computed(() => store.subnets.find(s => s.id === targetSubnetId.value))
  const subnetLabel = computed(() => subnet.value ? (subnet.value.name || subnet.value.cidr) : 'Subnet')

  function show (subnetId: string) {
    targetSubnetId.value = subnetId
    username.value = ''
    address.value = ''
    error.value = ''
    open.value = true
  }
  function close () {
    open.value = false
  }

  async function submit () {
    if (!username.value.trim()) {
      error.value = 'Username required'; return
    }
    if (username.value.trim().length === 0 || username.value.trim().length > 15) {
      error.value = 'Username too short or too long, username must be between 1 and 15 characters'; return
    }
    if (!subnet.value) {
      error.value = 'Subnet not found'; return
    }
    loading.value = true; error.value = ''
    try {
      const cidr = subnet.value.cidr
      const cfg = await backend.createPeer(username.value.trim(), cidr, address.value.trim() || undefined)
      if (!cfg) {
        error.value = backend.lastError || 'Failed to create peer'; return
      }
      // Fire global event to display configuration popup
      window.dispatchEvent(new CustomEvent('peer-created-with-config', { detail: { username: username.value.trim(), configuration: cfg } }))
      // After topology refresh, open the details for peer
      // Suppress immediate peer details popup to avoid overlapping configuration dialog
      open.value = false
    } catch (error_: any) {
      error.value = error_?.message || 'Unexpected error'
    } finally {
      loading.value = false
    }
  }

  defineExpose({ show, close })
</script>
