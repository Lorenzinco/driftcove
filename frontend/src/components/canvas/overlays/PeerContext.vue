<template>
  ...
  />
  <div v-if="menu.open" class="peer-menu-backdrop" @click="hideMenu" @mousedown.stop />
...
<script setup lang="ts">
  ...
  function openInfo (id: string) {
    hideMenu(); store.openPeerDetails(id)
  }
  ...
  onMounted(() => {
    window.addEventListener('keydown', onKey); window.addEventListener('peer-context-request-delete', onExternalDeleteRequest as any)
  })
  onUnmounted(() => {
    window.removeEventListener('keydown', onKey); window.removeEventListener('peer-context-request-delete', onExternalDeleteRequest as any)
  })
