<template>
  <v-dialog v-model="open" max-width="420">
    <v-card>
      ...
      <v-card-text>
        <template v-if="links.length > 0">
          <div class="text-body-2 mb-2">
            Links between <strong>{{ entityName(links[0].fromId) }}</strong> and
            <strong>{{ entityName(links[0].toId) }}</strong>
          </div>
          <v-select
            v-model="selectedId"
            density="comfortable"
                  ...
                  <div class="d-flex align-center ga-1">
                    <v-icon
                      :color="
                        item.raw.kind === 'p2p'
                          ? 'success'
                          : item.raw.kind === 'service'
                            ? 'primary'
                            : item.raw.kind === 'admin-p2p'
                              ? 'purple'
                              : item.raw.kind === 'admin-subnet-subnet'
                                ? 'purple'
                                : item.raw.kind === 'admin-peer-subnet'
                                  ? 'purple'
                                  : item.raw.kind === 'subnet-subnet'
                                    ? 'success'
                                    : 'secondary'
                      "
                      :icon="
                    ...
                    <v-icon
                      v-if="
                        item.raw.kind === 'p2p' &&
                        problematicLinkIds.has(item.raw.id)
                      "
                      color="warning"
      ...
      <v-card-actions class="justify-end ga-2">
        ...
        <v-btn color="error" :disabled="!selectedId" @click="openConfirmDialog"
          >Delete</v-btn
        >
      </v-card-actions>
    </v-card>
  ...
  <ConfirmCutLinkDialog
    ...
    :color="
      currentLink
        ? currentLink.kind === 'p2p'
          ? 'success'
          : currentLink.kind === 'admin-p2p'
            ? 'purple'
            : currentLink.kind === 'admin-subnet-subnet'
              ? 'purple'
              : currentLink.kind === 'admin-peer-subnet'
                ? 'purple'
                : currentLink.kind === 'membership'
                  ? 'secondary'
                  : currentLink.kind === 'subnet-subnet'
                    ? 'success'
                    : 'primary'
        : 'primary'
    "
            ...
            : currentLink.kind === 'admin-subnet-subnet'
              ...
              : currentLink.kind === 'admin-peer-subnet'
                ...
                : currentLink.kind === 'membership'
                  ...
                  : currentLink.kind === 'subnet-subnet'
                    ? 'subnet-subnet'
                    : currentLink.kind === 'subnet-service'
                      ? 'subnet-service'
                      : 'service-generic'
        : ''
    "
    ...
    :problematic="
      currentLink?.kind === 'p2p' && problematicLinkIds.has(currentLink.id)
    "
...
const selectItems = computed(() =>
  links.value.map((l) => {
    ...
    if (l.kind === "service") {
      ...
      const proto =
        svcRec && (svcRec as any).protocol
          ? ` ${(svcRec as any).protocol}`
          : "";
      return {
        value: l.id,
        kind: l.kind,
        label: `${from} -> ${to}${portStr} (${svcName}${proto})`,
        id: l.id,
      };
    }
    ...
    if (l.kind === "subnet-service") {
      ...
      const proto =
        svcName && host?.services?.[svcName]?.protocol
          ? ` ${host.services[svcName].protocol}`
          : "";
      const label = `${host?.name || from} → ${to} (${svcName || "service"}${proto})`;
  ...
  try {
    ...
    switch (link.kind) {
      ...
      case "admin-p2p": {
        ...
        if (admin && regular)
          await backend.disconnectAdminPeerFromPeer(admin.name, regular.name);
      }
      case "service": {
        const consumer = store.peers.find((p) => p.id === link.toId);
        const svcName = link.serviceName || "";
        if (consumer && svcName)
          await backend.disconnectPeerFromService(consumer.name, svcName);
      }
      case "membership": {
        const peer =
          store.peers.find((p) => p.id === link.fromId) ||
          store.peers.find((p) => p.id === link.toId);
        if (!peer) return;
      ...
      case "admin-subnet-subnet": {
        ...
        if (adminSubnet && targetSubnet)
          await backend.disconnectAdminSubnetFromSubnet(
            adminSubnet.cidr,
            targetSubnet.cidr,
          );
      }
      case "subnet-service": {
        ...
        const svcName = link.serviceName || "";
        if (subnet && svcName)
          await backend.disconnectSubnetFromService(subnet.cidr, svcName);
      }
      case "admin-peer-subnet": {
        ...
        if (adminPeer && subnet)
          await backend.disconnectAdminPeerFromSubnet(
            adminPeer.name,
            subnet.cidr,
          );
      }
