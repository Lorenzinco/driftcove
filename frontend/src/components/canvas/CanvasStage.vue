<template>
  <div class="fill-height p-4 canvas-wrapper" @contextmenu.prevent>
    ...
    <div v-if="indicator.visible" :style="indicatorStyle" class="tool-indicator">
...
const interactions = createInteractions(()=>store.peers, ()=>store.subnets, ()=>store.links, () => store.zoom);
...
const renderer = createRenderer(() => ({
  ...
  hoverLinkId: ui.hoverLinkId,
    ...
    tool: store.tool,
    connect: ui.connect,
    ghostSubnet: ui.ghostSubnet.active && store.tool==='add-subnet' ? ui.ghostSubnet : null,
  }
}));
...
function onResize(){ resizeToParent(); invalidate(); }

function onWheel(e:WheelEvent){
  e.preventDefault(); e.stopPropagation();
  const rect = canvasRef.value!.getBoundingClientRect();
  const mx = e.clientX - rect.left; const my = e.clientY - rect.top;
  const wx = (mx - store.pan.x)/store.zoom; const wy = (my - store.pan.y)/store.zoom;
  const factor = Math.exp(-e.deltaY * 0.0015);
  store.zoom = Math.min(2.5, Math.max(0.4, store.zoom * factor));
  store.pan.x = mx - wx * store.zoom; store.pan.y = my - wy * store.zoom;
  invalidate();
}

const click = reactive({ down:false, worldX:0, worldY:0, target:'', type:'' as ''|'peer'|'subnet'|'link', moved:false });

function syncSelection(sel: null | { type:'peer'|'subnet'|'link'; id:string }) {
  ui.selection = sel;
  if (!sel) { store.selection = null as any; return; }
  if (sel.type==='peer') {
...
function onMouseDown(e:MouseEvent){
  ...
  if (e.button===1 || (e.button===0 && e.shiftKey)) { store.pan.dragging=true; (store.pan as any).sx=sx; (store.pan as any).sy=sy; return; }
  ...
  click.down=true; click.worldX=pt.x; click.worldY=pt.y; click.target=peer.id; click.type='peer'; click.moved=false; (click as any).sx = sx; (click as any).sy = sy;
      ...
      const descendants = store.subnets.filter(s=> s.id!==subnet.id && s.cidr && subnet.cidr && cidrContains(subnet.cidr, s.cidr));
      ...
      interactions.drag.containedPeers = store.peers.filter(p=> {
  ...
  click.down=true; click.worldX=pt.x; click.worldY=pt.y; click.target=subnet.id; click.type='subnet'; click.moved=false; (click as any).sx = sx; (click as any).sy = sy;
    ...
    click.worldX=pt.x; click.worldY=pt.y; click.type=''; click.target=''; click.moved=false; (click as any).sx = sx; (click as any).sy = sy;
  }
  invalidate();
}
...
function onMouseMove(e:MouseEvent){
  const rect = canvasRef.value!.getBoundingClientRect();
  const sx = e.clientX - rect.left; const sy = e.clientY - rect.top; const pt = screenToWorld(sx, sy); (window as any).__lastPointerScreen = { x:sx, y:sy };
  if (store.pan.dragging){ store.pan.x += sx - (store.pan as any).sx; store.pan.y += sy - (store.pan as any).sy; (store.pan as any).sx=sx; (store.pan as any).sy=sy; invalidate(); return; }
    if (interactions.resizeDrag.active){
      const s = store.subnets.find(ss=> ss.id===interactions.resizeDrag.id); if (s){
        let { left, right, top, bottom } = interactions.resizeDrag;
        const minW = interactions.resizeDrag.minW; const minH = interactions.resizeDrag.minH;
    ...
    if (interactions.resizeDrag.edge.includes('s')) bottom = pt.y;
        ...
        const parents = store.subnets.filter(ps=> ps.id!==s.id && ps.cidr && s.cidr && cidrContains(ps.cidr, s.cidr));
        if (parents.length){
          parents.sort((a,b)=> parseInt(b.cidr.split('/')[1]) - parseInt(a.cidr.split('/')[1]));
          const parent = parents[0]; const margin=20;
          const pl = parent.x - parent.width/2 + margin; const pr = parent.x + parent.width/2 - margin;
    ...
    const margin = 26;
    const pl = s.x - s.width/2 + margin; const pr = s.x + s.width/2 - margin;
    const ptp = s.y - s.height/2 + margin; const pb = s.y + s.height/2 - margin;
    for (const p of store.peers){ if (p.subnetId === s.id){ if (p.x<pl) p.x=pl; if (p.x>pr) p.x=pr; if (p.y<ptp) p.y=ptp; if (p.y>pb) p.y=pb; } }
        click.moved = true;
      }
      invalidate(); return; }
  if (interactions.drag.active){
    if (interactions.drag.type==='peer'){
      const n = store.peers.find(p=>p.id===interactions.drag.id); if (n){ n.x = pt.x - interactions.drag.offsetX; n.y = pt.y - interactions.drag.offsetY; interactions.clampPeerToSubnet(n); }
    } else if (interactions.drag.type==='subnet') {
      const s = store.subnets.find(ss=>ss.id===interactions.drag.id); if (s){
        const oldX=s.x, oldY=s.y; s.x = pt.x - interactions.drag.offsetX; s.y = pt.y - interactions.drag.offsetY; let dx=s.x-oldX, dy=s.y-oldY;
        ...
        if (parents.length){
          ...
          const margin = 20;
            ...
            if (s.x - s.width/2 < pl){ s.x = pl + s.width/2; }
            if (s.x + s.width/2 > pr){ s.x = pr - s.width/2; }
            if (s.y - s.height/2 < ptp){ s.y = ptp + s.height/2; }
            if (s.y + s.height/2 > pb){ s.y = pb - s.height/2; }
            dx = s.x-oldX; dy = s.y-oldY;
        }
        for (const p of store.peers) if (interactions.drag.containedPeers.includes(p.id)){ p.x+=dx; p.y+=dy; }
        for (const sid of interactions.drag.containedSubnets){ const cs = store.subnets.find(x=>x.id===sid); if (cs){ cs.x+=dx; cs.y+=dy; } }
      }
    }
    if (click.down && !click.moved){ const dx=pt.x - click.worldX, dy=pt.y - click.worldY; if (Math.hypot(dx,dy) > 4/store.zoom) click.moved=true; }
    invalidate(); return;
  }
  if (click.down && !click.moved){ const dx=pt.x - click.worldX, dy=pt.y - click.worldY; if (Math.hypot(dx,dy) > 4/store.zoom) click.moved=true; }
  // Hover targets
  ...
  if (!interactions.drag.active && !interactions.resizeDrag.active){
    if (ui.hoverLinkId){
      ...
      if (edge){
        ...
        else if (edge === 'ne' || edge === 'en' || edge === 'sw' || edge === 'ws') cursor = 'nesw-resize';
        else if (edge === 'nw' || edge === 'wn' || edge === 'se' || edge === 'es') cursor = 'nwse-resize';
        (canvasRef.value as any).style.cursor = cursor;
      } else {
        (canvasRef.value as any).style.cursor = peer ? 'pointer' : 'default';
      }
    } else {
      (canvasRef.value as any).style.cursor = (peer || ui.hoverLinkId) ? 'pointer' : 'default';
    }
  }
  // Connect ghost line
  ...
  if (store.tool==='connect') {
    ...
    indicator.icon = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><line x1="20" y1="4" x2="8.12" y2="11"/><line x1="14.47" y1="14.48" x2="20" y2="20"/><line x1="8.12" y1="13" x2="20" y2="20"/></svg>';
...
function onMouseUp(){
  ...
  if (click.down && !click.moved && click.type==='' && store.tool==='add-subnet') {
...
function clearGhostSubnet(){ ui.ghostSubnet.active=false; ui.ghostSubnet.frozen=false; invalidate(); }
...
.tool-indicator { position:fixed; z-index:50; display:flex; flex-direction:column; align-items:center; gap:2px; font-family: Roboto, sans-serif; font-size:11px; font-weight:500; color:#fff; text-shadow:0 1px 2px rgba(0,0,0,0.6); }
.tool-indicator .icon { width:24px; height:24px; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.55); backdrop-filter:blur(2px); border:1px solid rgba(255,255,255,0.18); border-radius:6px; box-shadow:0 2px 6px rgba(0,0,0,0.4); }
.tool-indicator .label { padding:2px 6px; background:rgba(0,0,0,0.45); border:1px solid rgba(255,255,255,0.15); border-radius:5px; }
</style>
